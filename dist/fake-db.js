...
exports.decoratePost = exports.setVote = exports.addComment = exports.getSubs = exports.deletePost = exports.editPost = exports.addPost = exports.getVoteForPostByUser = exports.getPost = exports.getPosts = exports.getUserByUsername = exports.getUser = exports.debug = void 0;
// @ts-nocheck
const users = {
    1: {
        id: 1,
...
function getVoteForPostByUser(post_id, user_id) {
    return votes.find((vote) => vote.post_id === Number(post_id) && vote.user_id === Number(user_id));
}
exports.getVoteForPostByUser = getVoteForPostByUser;
function decoratePost(post) {
    if (!post) {
        return null;
    }
    const unknownUser = { id: 0, uname: "Unknown", password: "" };
    post = Object.assign(Object.assign({}, post), { creator: users[Number(post.creator)] || unknownUser, votes: getVotesForPost(post.id), comments: Object.values(comments)
            .filter((comment) => comment.post_id === post.id)
            .map((comment) => (Object.assign(Object.assign({}, comment), { creator: users[Number(comment.creator)] || unknownUser }))) });
    return post;
}
...
function setVote(post_id, user_id, value) {
    ...
    const voteIndex = votes.findIndex((vote) => vote.post_id === Number(post_id) && vote.user_id === Number(user_id));
